...
class TestLlamaServiceIntegration(unittest.TestCase):
    ...
    def test_統合テスト_正常な検索(self):
        for query, expected in self.test_cases:
            ...
            # assert
            print(f"Testing query: {query}, Expected: {result.node.metadata['question']}")
    ...
    def test_cases(self):
        ...
        return [
            ...
            ,("配偶者を扶養に入れてい", "対配偶者を扶養に入れない場合でも、住民票は世帯全体で考えてください。")
            ,("配偶者を扶養に入れていないですが、必要ですか？", "対配偶者を扶養に入れない場合でも、住民票は世帯全体で考えてください。")
            ...
            ,("会費はどのように納めれば良いでしょうか？", "現状、初回会費は手動での振り込みをお願いしておりますが、次回以降はご登録いただいたクレジットカードでの自動決済(毎月15日)となります。 ※次回以降も手動振り込みをご希望の場合は、事前にご相談ください。")
